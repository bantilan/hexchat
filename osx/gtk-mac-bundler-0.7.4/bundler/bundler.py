...
class Bundler:
    def __init__(self, project):
        ...
        self.plist = load_plist(plist_path)
    ...
    def create_pango_setup(self):
        ...
        try:
            ...
            print("Skipping pango setup (pango_module_version not found).")
            return
        if shutil.which("pango-querymodules") is None:
            print("Skipping pango setup (pango-querymodules not found).")
            return
        ...
        if ver_tuple(module_version) < ver_tuple('1.8.0'):
            ...
            for globbed_source in glob.glob(source):
                try:
                    if os.path.isdir(globbed_source):
                        ...
                        utils.makedirs(dest)
                        shutil.copytree(str(globbed_source), str(dest),
                                        dirs_exist_ok=True, symlinks=True)
                    else:
                        #print "file: %s => %s" % (globbed_source, dest)
                        if os.path.isdir(dest) or dest.endswith(os.sep):
                            dest_dir = dest
                            dest_file = os.path.join(dest, os.path.basename(globbed_source))
                        else:
                        ...
                        if os.path.exists(dest_file):
                            continue
                        shutil.copy2(str(globbed_source), str(dest_file))
                except EnvironmentError as e:
                    if e.errno == errno.ENOENT:
                        print("Warning, source file missing: " + globbed_source)
                    elif e.errno == errno.EEXIST:
        ...
        for program in translations:
            ...
            for root, trees, files in os.walk(source):
                for file in filter(name_filter, files):
                    ...
                    self.copy_path(Path(path, program.dest))
